...
export function Badge({ children }) {
  ...
  return (
    <span
      className={`badge ${/completed|verified|normal|active/i.test(value) ? "green" : /failed|cancelled|closed/i.test(value) ? "red" : "amber"}`}
    >
      {value.replaceAll("_", " ")}
    </span>
  );
...
export function Empty({
  ...
  text = "Your updates will appear here as you move through procurement.",
  action,
}) {
  return (
    <div className="empty">
...
export function Table({ columns, rows, empty }) {
  return rows.length ? (
    ...
    <Empty title={empty || "No records yet"} />
  );
}
